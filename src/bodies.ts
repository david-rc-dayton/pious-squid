...
export function precession(epoch: Epoch): [number, number, number] {
  ...
  const theta = evalPoly(t, [0.0, 0.556753, -0.0001185, -1.16e-5]) * c.DEG2RAD;
...
export function nutation(epoch: Epoch): [number, number, number] {
  ...
  const sunAnom =
    (evalPoly(
      t,
      [357.52910918, 99.0 * degRev + 359.0502911, -0.0001537, 3.8e-8]
    ) %
      degRev) *
    c.DEG2RAD;
  const moonLat =
    (evalPoly(
      t,
      [93.27209062, 1342.0 * degRev + 82.0174577, -0.003542, -2.88e-7]
    ) %
      degRev) *
    c.DEG2RAD;
  const sunElong =
  ...
  const meanEpsilon =
    (evalPoly(t, [23.439291, -0.013004, -1.64e-7, 5.04e-7]) % degRev) *
    c.DEG2RAD;
...
export function moonPosition(epoch: Epoch): Vector {
  ...
  const pllx =
    ((0.9508 +
      ...
      0.0028 * cos((269.9 + 954397.7 * jCent) * c.DEG2RAD)) %
      360) *
    c.DEG2RAD;
  ...
  const rJ =
    rMag *
    (cos(obq) * cos(phiEcl) * sin(lamEcl) - sin(obq) * sin(phiEcl)) *
    c.EARTH_RAD_EQ;
  const rK =
    rMag *
    (sin(obq) * cos(phiEcl) * sin(lamEcl) + cos(obq) * sin(phiEcl)) *
    c.EARTH_RAD_EQ;
