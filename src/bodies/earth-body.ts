...
export const RADIUS_EQUATOR = 6378.137;

/** Earth coefficient of flattening. */
export const FLATTENING = 1 / 298.257223563;

/** Earth rotation vector, in radians per second. */
export const ROTATION = new Vector3D(0, 0, 7.2921158553e-5);

/** Earth polar radius, in kilometers. */
export const RADIUS_POLAR = RADIUS_EQUATOR * (1 - FLATTENING);

/** Earth mean radius, in kilometers. */
export const EARTH_RAD_MEAN = (2 * RADIUS_EQUATOR + RADIUS_POLAR) / 3;

/** Earth eccentricity squared. */
export const ECCENTRICITY_SQUARED = FLATTENING * (2 - FLATTENING);
 ...
 */
export function precession(epoch: EpochUTC): [number, number, number] {
 ...
 * @param epoch satellite state epoch
 */
export function nutation(epoch: EpochUTC): [number, number, number] {
  const r = 360;
  const t = epoch.toTDB().toJulianCenturies();
