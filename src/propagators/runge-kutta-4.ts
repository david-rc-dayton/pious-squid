...
export class RungeKutta4 implements IPropagator {
   ...
   * in the model argument, options are merged from: DEFAULT_MODEL_TWOBODY
   *
  ...
  /**
   * Create a new RungeKutta4 propagator object. If values are not specified
   * in the model argument, options are merged from: DEFAULT_MODEL
   *
   * @param state satellite state
   * @param model propagator options
   */
  ...
  private integrate(step: number): J2000 {
    ...
    const k2 = drv(epoch.roll(step / 2), posVel.add(k1.scale(step / 2)));
    const k3 = drv(epoch.roll(step / 2), posVel.add(k2.scale(step / 2)));
    const k4 = drv(epoch.roll(step), posVel.add(k3.scale(step)));
    const [ri, rj, rk, vi, vj, vk] = posVel.add(
      k1
        .add(k2.scale(2))
        .add(k3.scale(2))
        .add(k4)
